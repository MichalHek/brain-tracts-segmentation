function [kappatauttnnbb]=feature_CurvatureTorsion(dataset)
...
taulist1 = [];
for i = 1:FN
rr = zeros(length(x(i,:)),3);
rr(:,1) = x(i,:); rr(:,2) = y(i,:); rr(:,3) = z(i,:);
rr = rr';
...
% kappalist = kappalist(~isnan(kappalist));
% taulist = taulist(~isnan(taulist));
% ttlist=ttlist(~isnan(ttlist(:))); % tangent
% nnlist=nnlist(~isnan(nnlist(:))); % normal
% bblist=bblist(~isnan(bblist(:))); % binormal
kappalist1 = [kappalist1,kappalist];
taulist1 = [taulist1,taulist];
% ttlist1 = [ttlist1;ttlist];
% nnlist1 = [nnlist1;kappalist];
% bblist1 = [bblist1;bblist];
end
kappalist1 = kappalist1(2:end-1,:)';
taulist1 = taulist1(3:end-1,:)';

% kappalist_quarter_length = floor(length(kappalist(:,1))/10);
% taulist_quarter_length = floor(length(taulist(:,1))/10);
% ttlist_quarter_length = floor(length(ttlist(:,1))/10);
% nnlist_quarter_length = floor(length(nnlist(:,1))/10);
% bblist_quarter_length = floor(length(bblist(:,1))/10);
% 
% kappalist_quarter = zeros(10,1);
% taulist_quarter = zeros(10,1);
% ttlist_quarter = zeros(10,1);
% nnlist_quarter = zeros(10,1);
% bblist_quarter = zeros(10,1);
% 
% for i = 0:9
%     if i<9
%         kappalist_quarter(i+1,:) = mean(kappalist(kappalist_quarter_length*i+1:kappalist_quarter_length*(i+1),:));
%         taulist_quarter(i+1,:) = mean(taulist(taulist_quarter_length*i+1:taulist_quarter_length*(i+1),:));
%         ttlist_quarter(i+1,:) = mean(ttlist(ttlist_quarter_length*i+1:ttlist_quarter_length*(i+1),:));
%         nnlist_quarter(i+1,:) = mean(nnlist(nnlist_quarter_length*i+1:nnlist_quarter_length*(i+1),:));
%         bblist_quarter(i+1,:) = mean(bblist(bblist_quarter_length*i+1:bblist_quarter_length*(i+1),:));
%     else
%         kappalist_quarter(i+1,:) = mean(kappalist(kappalist_quarter_length*i+1:length(kappalist(:,1)),:));
%         taulist_quarter(i+1,:) = mean(taulist(taulist_quarter_length*i+1:length(taulist(:,1)),:));
%         ttlist_quarter(i+1,:) = mean(ttlist(ttlist_quarter_length*i+1:length(ttlist(:,1)),:));
%         nnlist_quarter(i+1,:) = mean(nnlist(nnlist_quarter_length*i+1:length(nnlist(:,1)),:));
%         bblist_quarter(i+1,:) = mean(bblist(bblist_quarter_length*i+1:length(bblist(:,1)),:));
%     end
% end
% kappalist_quarter = kappalist_quarter';
% taulist_quarter = taulist_quarter';
% ttlist_quarter = ttlist_quarter';
% nnlist_quarter = nnlist_quarter';
% bblist_quarter = bblist_quarter';
% 
% kappalist = kappalist_quarter(:)';
% taulist = taulist_quarter(:)';
% ttlist = ttlist_quarter(:)';
% nnlist = nnlist_quarter(:)';
% bblist = bblist_quarter(:)';
% 
kappatauttnnbb = [kappalist1,  taulist1];
end
