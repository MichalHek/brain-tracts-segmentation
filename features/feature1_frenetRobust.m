function [kappatau]=feature_frenetRobust(rr)
...
kappalist_quarter_length = floor(length(kappalist(:,1))/4);
taulist_quarter_length = floor(length(taulist(:,1))/4);
kappalist = kappalist(~isnan(kappalist));
taulist = taulist(~isnan(taulist));
kappalist_quarter = zeros(4,1);
taulist_quarter = zeros(4,1);
for i = 0:3
    if i<3
